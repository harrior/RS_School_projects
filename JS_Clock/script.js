...
function addPomodoro(){
    const shelf = document.querySelector('.pomodoro-shelf');
    let newPomodoro = document.createElement("div")
    newPomodoro.className = 'pomodoro';
    shelf.appendChild(newPomodoro);
}

function showDescription(){
    console.log("Score: 30/30\n" +
        "Данное приложение реализует простой таймер для методики Pomodoro.\n" +
        "Умеет засекать время, по методике 25 минут, но в данной версии всего две (минуты для удобства проверки)\n" +
        "Если таймер дойдет до конца, то под ним появится заполненная помидорка.\n\n" +
        "[x] Разобрался в исходном коде примера и воспроизвел его самостоятельно. Добавлен вывод даты и времени текстом.\n" +
        "[x] Реализован таймер\n" +
        ...
        "Если таймер не был прерван то добавляется выполненный помидор в список");
}